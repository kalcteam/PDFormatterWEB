...
export default function PedidosPage() {
  ...
  const cargarDatos = useCallback(async (estado: FiltroEstado, page: number, q = "") => {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      setStats(MOCK_STATS)
      setTotalMostrado(filtrados.length)
      setTotalPaginas(16)
      setLoading(false)
      return
    }
    try {
      ...
      if (resPedidos.ok) {
        ...
        setTotalMostrado(json.data?.length ?? 0)
      }
      if (resStats.ok) {
  ...
  // Apply extra filters to mock data locally
              ...
              >
                ...
                {f.count !== undefined && (
                  <span
                    ...
                    style={{
                      background: filtro === f.key ? "#ebe4d8" : "#f3eee6",
                      color: "#847a6f",
                    }}
                  >
                    {f.count}
            ...
            <div className="relative" ref={filtrosRef}>
              ...
              >
                ...
                {filtrosActivosCount > 0 && (
                  <span
                    className="size-4 rounded-full text-[10px] flex items-center justify-center font-medium"
                    style={{ background: "#f57a26", color: "#ffffff" }}
                  >
                    {filtrosActivosCount}
                  </span>
          ...
          <table className="w-full min-w-[640px]">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-sm" style={{ color: "#b8aea1" }}>
                ...
                pedidosFiltrados.map((p, i) => (
                  <tr
                    ...
                    onMouseLeave={e => (e.currentTarget.style.background = "#ffffff")}
                    onClick={() => router.push(`/empleado/pedidos/${p.id}`)}
                  >
                    ...
                    <td className="px-4 py-3 text-sm" style={{ color: "#4a423b" }}>
                      {p.num_productos ?? <span style={{ color: "#ddd4c4" }}>—</span>}
                    </td>
                    <td className="px-4 py-3">
                      <EstadoBadge estado={p.estado} />
                    </td>
                    <td className="px-4 py-3">
                      {p.ia_confianza ? <IaConfianzaBadge nivel={p.ia_confianza} /> : <span style={{ color: "#ddd4c4" }}>—</span>}
                    </td>
                    <td className="px-4 py-3 text-xs" style={{ color: "#b8aea1" }}>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-[8px] transition-colors"
                style={{
                  border: "1px solid #ebe4d8", background: "#ffffff", color: pagina <= 1 ? "#ddd4c4" : "#4a423b",
                  cursor: pagina <= 1 ? "not-allowed" : "pointer",
                }}
                onMouseEnter={e => { if (pagina > 1) (e.currentTarget as HTMLElement).style.background = "#f3eee6" }}
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-[8px] transition-colors"
                style={{
                  border: "1px solid #ebe4d8", background: "#ffffff", color: pagina >= totalPaginas ? "#ddd4c4" : "#4a423b",
                  cursor: pagina >= totalPaginas ? "not-allowed" : "pointer",
                }}
                onMouseEnter={e => { if (pagina < totalPaginas) (e.currentTarget as HTMLElement).style.background = "#f3eee6" }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "#ffffff" }}
              >
